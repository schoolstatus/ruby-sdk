...
module Bandwidth
  ...
  class AddressesResponse < BaseModel
    ...
    def self.from_hash(hash)
      ...
      payload = AddressesPayload.from_hash(hash['payload']) if hash['payload']
      pagination_links = PaginationLinks.from_hash(hash['paginationLinks']) if
        hash['paginationLinks']
      total_count = hash['totalCount']

      # Create object from extracted values.
      AddressesResponse.new(payload,
                            pagination_links,
                            total_count)
    end

    def self.from_element(root)
      payload = XmlUtilities.from_element(root, 'Addresses', AddressesPayload)
    ...
    def to_xml_element(doc, root_name)
      ...
      XmlUtilities.add_as_subelement(doc, root, 'Addresses', payload)
