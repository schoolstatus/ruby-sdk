# bandwidth
#
# This file was automatically generated by APIMATIC v2.0
# ( https://apimatic.io ).

module Bandwidth
  # BillingReportArchiveRetrievalResponse Model.
  class BillingReportArchiveRetrievalResponse < BaseModel
    # TODO: Write general description for this method
    # @return [String]
    attr_accessor :report_status

    # TODO: Write general description for this method
    # @return [String]
    attr_accessor :description

    # A mapping from model property names to API property names.
    def self.names
      @_hash = {} if @_hash.nil?
      @_hash['report_status'] = 'reportStatus'
      @_hash['description'] = 'description'
      @_hash
    end

    def initialize(report_status = nil,
                   description = nil)
      @report_status = report_status
      @description = description
    end

    # Creates an instance of the object from a hash.
    def self.from_hash(hash)
      return nil unless hash

      # Extract variables from the hash.
      report_status = hash['reportStatus']
      description = hash['description']

      # Create object from extracted values.
      BillingReportArchiveRetrievalResponse.new(report_status,
                                                description)
    end

    def self.from_element(root)
      report_status = XmlUtilities.from_element(root, 'ReportStatus', String)
      description = XmlUtilities.from_element(root, 'Description', String)

      new(report_status,
          description)
    end

    def to_xml_element(doc, root_name)
      root = doc.create_element(root_name)

      XmlUtilities.add_as_subelement(doc, root, 'ReportStatus', report_status)
      XmlUtilities.add_as_subelement(doc, root, 'Description', description)

      root
    end
  end
end
